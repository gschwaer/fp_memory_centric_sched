...
#ifdef CONFIG_COLORING
	struct jailhouse_memory_colored const *col_mem;
	for_each_col_mem_region(col_mem, cell->config, n) {
		if (col_mem->memory.flags & (JAILHOUSE_MEM_IO | JAILHOUSE_MEM_COMM_REGION))
			continue;
		col_manage_functions dcache_data;
		dcache_data.flush = flush;
		manage_colored_regions(*col_mem, cell, &dcache_data, DCACHE);
	}
#endif
