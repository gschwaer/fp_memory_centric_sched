...
/**
 *  This enum defines possible memory arbitration states assigned by the
 *  hypervisor.
 */
typedef enum {
  MEMORY_TOKEN_NONE,           /**< memory access not permitted */
  MEMORY_TOKEN_REQUESTED,      /**< memory access requested but not granted */
  MEMORY_TOKEN_ACQUIRED,       /**< memory access granted */
} OsEE_mem_arbit_status;
...
typedef struct {
   ...
   *  Tasks suspended from interrupt while in execution or actively suspended
   *  by calling DPREM_suspend_running_task go here. */
  P2VAR(OsEE_SN, TYPEDEF, OS_APPL_DATA)           p_suspended_sn;
  /** The state of the memory arbitration as signaled from the hypervisor.
   *  Maintained by dprem.c */
  VAR(OsEE_mem_arbit_status volatile, TYPEDEF)    mem_arbit_status;
} OsEE_CCB;
