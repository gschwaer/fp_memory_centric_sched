		     ...
		     unsigned int count_event)
{
	...
	case SGI_EVENT:
		...
		break;
	default:
		printk("WARN: unknown SGI received %d cell %d\n", irqn, this_cpu_id());
	}
}

/*
 * Handle the maintenance interrupt and optionally the TDMA interrupts, the rest
 * is injected into the cell. Return true when the IRQ has been handled by the
 * hyp.
 */
...
#ifdef CONFIG_JAILHOUSE_TDMA
	else if (tdma_handle_interrupt(cpu_data, irqn)) {
		return true;
	}
#endif
...
{
	int res = arm_paging_cell_init(cell);

#ifdef CONFIG_JAILHOUSE_TDMA
	tdma_on_cell_create(cell);
#endif
