...
void osEE_aarch64_isr_wrapper(OsEE_ISR_CTX * p_isr_ctx)
{
  ...
  while ( 1 ) {
    ...
    // SGI/IPI
    // - bits [10:12] encode the source cpu id
    // - bits[9:0] are the interrupt id
    irqn_id_only = irqn & 0x3FF;
    //printk("INT %u from %u\n", irqn_id_only, irqn);

    {
...
#endif /* !OSEE_SINGLECORE */
      {
        ...
        if (irqn_id_only < OSEE_GIC_MIN_SPI_ID) {
          CoreIdType const cpu_id = osEE_get_curr_core_id();
          p_hnd = &osEE_aarch64_ppi_isr_vectors[cpu_id][irqn_id_only];
        } else {
          p_hnd = &osEE_aarch64_spi_isr_vectors[irqn_id_only - OSEE_GIC_MIN_SPI_ID];
        }

        if (p_hnd->cat == OSEE_ISR_CAT_2) {
          TaskType const tid = p_hnd->hnd.tid;
          if (tid != INVALID_TASK) {
            osEE_activate_isr2(tid, irqn);
          } else {
            osEE_gicc_eoi(irqn);
          }
          ...
          osEE_gicc_eoi(irqn);
        } else { // p_hnd->cat == OSEE_ISR_CAT_INVALID
          osEE_gicc_eoi(irqn);
        }
      }
    }
