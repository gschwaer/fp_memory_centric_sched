 ...
 *
 * NOTE: Add "mem=7000M vmalloc=512M" to the kernel command line.
 *
 */
...
struct {
	...
	.header = {
		...
		.platform_info = {
			...
			.coloring_desc = {
				/* PAGE_SIZE set to the minimum possibile
				 * value so that we support the maximum
				 * number of colors for this SoC: 32
				 */
				.fragment_unit_size = 0x1000,
				/* Way size: 128 KB (2MB/16)*/
				.fragment_unit_offset = 0x20000,
				/* Cache size: 2 MB*/
				.cachesize = 0x200000,
			},
		},
		.root_cell = {
			.name = "Jetson-TX2-AGI-Root",
	...
	.mem_regions = {
		...
		/* System RAM */ {
			...
			.size = 0x1b5a00000, // 7 GiB
			.flags = JAILHOUSE_MEM_READ | JAILHOUSE_MEM_WRITE |
				JAILHOUSE_MEM_EXECUTE,
		},
		/* Inmates */ {
			.phys_start = 0x23da00000,
			.virt_start = 0x23da00000,
			.size = 0xc000000, // 192 MiB (32MiB each)
			.flags = JAILHOUSE_MEM_READ | JAILHOUSE_MEM_WRITE |
				JAILHOUSE_MEM_EXECUTE,
		},
		/* Persistent RAM */ {
		...
		/* IVHSMEM example */ {
			.phys_start = 0x245a00000,
			.virt_start = 0x245a00000,
			.size = 0x10000,
			.flags = JAILHOUSE_MEM_READ | JAILHOUSE_MEM_WRITE |
				JAILHOUSE_MEM_EXECUTE,
		},
