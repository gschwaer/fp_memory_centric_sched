...
static void cell_destroy_internal(struct per_cpu *cpu_data, struct cell *cell)
{
	...
	for_each_col_mem_region(col_mem, cell->config, n) {
		col_manage_functions destroy_functions;
		struct destroy_functions functions = {
			arch_unmap_memory_region,
			remap_to_root_cell,
		};
		destroy_functions.destroy_f = functions;
		manage_colored_regions(*col_mem, cell, &destroy_functions, DESTROY);
	}
#endif
...
static int cell_create(struct per_cpu *cpu_data, unsigned long config_address)
{
	...
	const struct jailhouse_memory_colored *col_mem;
	for_each_col_mem_region(col_mem, cell->config, n) {
		col_manage_functions create_functions;
		struct create_functions functions = {
			arch_map_memory_region,
			mmio_subpage_register,
			unmap_from_root_cell,
		};
		create_functions.create_f = functions;
		err = manage_colored_regions(*col_mem, cell, &create_functions, CREATE);
		if(err)
			goto err_destroy_cell;
	}
	printk("Mapping colored regions\n");
#endif
...
#ifdef CONFIG_COLORING
		const struct jailhouse_memory_colored *col_mem;
		col_manage_functions functions;

		functions.unmap_root_f = unmap_from_root_cell;
		for_each_col_mem_region(col_mem, cell->config, n) {
			err = manage_colored_regions(*col_mem, cell, &functions, START);
			if(err)
				goto out_resume;
		}
#endif
...
static int cell_set_loadable(struct per_cpu *cpu_data, unsigned long id)
{
	...
	functions.remap_root_f = remap_to_root_cell;
	for_each_col_mem_region(col_mem, cell->config, n) {
		err = manage_colored_regions(*col_mem, cell, &functions, LOADABLE);
		if(err)
			goto out_resume;
	}
#endif
 ...
 * @param arg3		Third hypercall argument.
 *
 * @return Value that shall be passed to the caller of the hypercall on return.
 *
 * @note If @c arg1 , @c arg2 and @c arg3 are valid depends on the hypercall code.
 */
long hypercall(unsigned long code, unsigned long arg1, unsigned long arg2, unsigned long arg3)
{
...
#ifdef CONFIG_JAILHOUSE_TDMA
	case JAILHOUSE_HC_MEMORY_ARBITRATION:
		return tdma_hypercall(cpu_data, arg1);
#endif

#ifdef CONFIG_JAILHOUSE_FIXED_PRIORITY
	case JAILHOUSE_HC_MEMORY_ARBITRATION:
		return fixed_priority_hypercall(cpu_data, arg1);
#endif
